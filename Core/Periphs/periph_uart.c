...
static bool init_okay_s = false; // prevents public methods from being used if init had failed.
...
bool periph_uart_init(UART_HandleTypeDef *huart)
{
	...
	huart_s = *huart; // de-reference it to set the local variable
...
void periph_uart_handle_rx(UART_HandleTypeDef *huart)
{
	...
	// if didn't receive the carriage return increment the rx_buffer and add the next character
	if (rx_data_s != '\r')
	{
	...
	// otherwise we did get a c. return, check against expected string
	...
	HAL_UART_Transmit(huart, tx_buffer_s, (uint16_t)MAX_BUFFER_LEN, 100);
	// clear both buffers
